...
my_sens_heatmap <- ggplot(data_melt_exp2, aes(x = AttenuationCoefficient, y = variable, fill = value)) +
  geom_tile(color = "white") +
  geom_text(aes(label = value), size = 4, family = "sans", color = "white", fontface = "bold") +  # Add numbers inside blocks
  scale_fill_viridis(option = "D", direction = 1, name = "Number of Larvae") +
  labs(title = "",
       x = "Light Attenuation Coefficient", 
       y = NULL) +  # Updated axis labels
  theme_minimal() +
  theme(
    text = element_text(size = 12),  # Set text size to 20
    axis.title = element_text(size = 12, margin = margin(r = 40)),  # Axis titles
...
ggsave("D:/_Sarah/paperbackup/2025/Alife/Figures/exp2-sens.pdf", plot = my_sens_heatmap, width = 8, height = 6, dpi = 300, device = cairo_pdf)
